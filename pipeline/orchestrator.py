    ...
    def answer(self, query: str) -> RAGResponse:
        ...
        for attempt in range(2):
            ...
            if pending_grounding:
                ...
                # MUST be True here, not False — output_validator requires
                # refusal_reason to be set whenever grounded=False. This is
                # only a *tentative* True to pass structural validation; the
                # grounding check below (or the fallback) determines the
                # real, final value before the response is returned. If
                # grounding ultimately fails, the retry/refuse paths set
                # grounded=False together with a real refusal_reason, so the
                # invariant is never violated on the way out.
            ...
            # ── Python-side citation fallback (NO extra LLM call) ───────────
            # If the model gave no usable tags, don't fail it for that alone.
            # Check the answer against the FULL retrieved set instead of an
            # empty cited set. If it grounds, Python assigns the supporting
            # chunk_ids as sources itself. Grounding is still real — the
            # answer must actually pass grounding.check() against genuine
            # chunk text, it just isn't gated on perfect tag formatting.
            ...
            # ── Normal grounding check against model-cited chunks ───────────
            t0 = time.perf_counter()
